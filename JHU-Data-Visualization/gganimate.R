...
ggplot(mtcars, aes(x=factor(cyl), y=mpg)) + #Factor turns numeric to categorical. Could also use "group = cyl" here - example below.
  geom_boxplot()
...
?transition_states #good with categorical vars

###Example of transition_time

?transition_time #good with continuous vars
...
###transition_layers
?transition_layers #good for plotting layer by layer, following sequence of commands in the code (instead of all at once)
...
#####entering and exiting -- makes transitions smoother
...
#######shadowing -- retains old views to be able to show transition over time
