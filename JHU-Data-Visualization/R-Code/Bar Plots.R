...
cel %>% 
  ...
  #Ahmed
  cel %>%  filter(congress==115) %>% ggplot(aes(x=dem)) + geom_bar()
...
cel %>% filter(congress==115) %>% ggplot(aes(x=st_name))+geom_bar()
  ...
  #Ahmed
  l=ggplot(filter(cel,congress==115),aes(x=dem))
  l + geom_bar()
...
cel %>% filter(congress==115) %>% ggplot(aes(y=st_name))+geom_bar()
  ...
  #Ahmed
  l= ggplot(filter(cel, congress==115), aes(y=st_name))
  l+ geom_bar()
...
cel %>% filter(congress==115)%>% ggplot(aes(x=party))+
  ...
  #Ahmed
  l= ggplot(filter(cel, congress==115), aes(x=party))
  l+ geom_bar()
...
cel %>% filter(congress==115)%>% ggplot(aes(x=party))+
  ...
  #Ahmed
  l= ggplot(filter(cel, congress==115), aes(x=party))
  l+ geom_bar() + labs(title = "Party Mix", x= "Party", y="Count of Members")
...
###add colors for the two different bars
  ...
  #Ahmed
  l= ggplot(filter(cel, congress==115), aes(x=party, fill = party))
  l+ geom_bar() + labs(title = "Party Mix", x= "Party", y="Count of Members")
...
fruit_bowl_summary <- fruit_bowl %>% 
  ...
  fruit_bowl_grouped = fruit_bowl%>% group_by(fruits)
  fruit_bowl_summary = summarize(fruit_bowl_grouped, "count"=n())
...
fruit_bowl_summary$proportion <- fruit_bowl_summary$count/sum(fruit_bowl_summary$count)
    ...
    proportion = fruit_bowl_summary$count/sum(fruit_bowl_summary$count)
    fruit_bowl_summary = cbind(fruit_bowl_summary,proportion)
