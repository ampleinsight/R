...
dim(cces) #Dimensions of table (Row and Column Counts)
dim(women)

table(cces$gender) #Cross Tabs
...
cces$party <- party #Replaces original column with the recoded one
...
cces <- test #Replaces original tibble with one that has new column name
cces$trump_approval
...
##### reorder rows by column values -- Sorts the data
...
cces %>% summarise (mean_pid = mean(pid7))
