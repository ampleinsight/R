...
####jitter adds random noise to the data to avoid overplotting (same point has multiple values: ex: 5 senators, all with same seniority, all with same # of bills passed -- jitter moves each point by a tiny random bit to show all 5 on the chart instead of just the one point that would otherwise show)
ggplot(fig115,aes(x=seniority,y=all_pass))+
  geom_jitter()
...
ggplot(fig115,aes(x=seniority,y=all_pass))+
  geom_jitter()+
  labs(x="Seniority",y="Bills Introduced",title="Seniority and Bills Passed in the 115th Congress")
...
fig115 = cel %>% filter(congress == 115) %>% select ("seniority", "all_pass", "dem")
