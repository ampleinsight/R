...
summarise(FAO_Pop_Max_Gp_Region, Total_Pop = sum(Value, na.rm = TRUE))
...
ggplot(FAO_Pop_Max,aes(x=Region,y = Value, fill=Region))+
  ...
  labs(x="Population",y="Region",
       title="Population Distribution by Region",caption="Source: UN Food and Agriculture Organization")


#Ridgeline plot? 
library(ggridges)

#How about we show world population over time here? :) - Scale changes very slowly...nothing interesting?
  
#How about: By economy level -- how has nourishment changed over time?
  #Also can show if nourishment levels differe by economy

FAO_Pop_World_Time = FAO %>% 
  filter(Indicator ==  "Total population" & Regions.Subregions.Countries == "WORLD") %>%
  select(Region, Year, Value)

FAO_Pop_Time = FAO_Pop %>% group_by(Region) %>%
  select(Region, Year, Value)

ggplot(FAO_Pop_World_Time, aes(x = Value, y = Year)) +
  geom_density_ridges ()+
  xlim(0, 300)+
  theme_ridges()+
  theme(legend.position = "none")

ggplot(FAO_Pop_Time, aes(x=Region, y=Value)) +
  geom_point()


unique(my_data$variable)
names(FAO)
class(FAO)
vignette("tibble")
